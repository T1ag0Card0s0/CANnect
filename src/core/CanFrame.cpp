...
uint32_t CanFrame::getCanId() const { return m_canId; }

uint8_t CanFrame::getDLC() const { return m_dlc; }

const uint8_t *CanFrame::getData() const { return m_data.data(); }

uint8_t *CanFrame::getData() { return m_data.data(); }

void CanFrame::setCanId(uint32_t canId) { m_canId = canId; }

void CanFrame::setDLC(uint8_t dlc) { m_dlc = dlc > Can_MAX_DATA_LENGTH ? Can_MAX_DATA_LENGTH : dlc; }
